...
accept_file = open("unit_tests/inputs/1_28a_accepts.txt", "r")
reject_file = open("unit_tests/inputs/1_28a_rejects.txt", "r")
...
### Test Inputs ###
from assignment04_main import prob_1_28a

for w in accept_list:
    if (w not in prob_1_28a):
        print(f'UNEXPECTED RESULT ON INPUT: \'{w}\'')
    assert w in prob_1_28a

for w in reject_list:
    if (w in prob_1_28a):
        print(f'UNEXPECTED RESULT ON INPUT: \'{w}\'')
    assert w not in prob_1_28a
