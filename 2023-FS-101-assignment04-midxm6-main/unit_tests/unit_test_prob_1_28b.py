...
accept_file = open("unit_tests/inputs/1_28b_accepts.txt", "r")
reject_file = open("unit_tests/inputs/1_28b_rejects.txt", "r")
...
### Test Inputs ###
from assignment04_main import prob_1_28b

for w in accept_list:
    if (w not in prob_1_28b):
        print(f'UNEXPECTED RESULT ON INPUT: \'{w}\'')
    assert w in prob_1_28b

for w in reject_list:
    if (w in prob_1_28b):
        print(f'UNEXPECTED RESULT ON INPUT: \'{w}\'')
    assert w not in prob_1_28b
