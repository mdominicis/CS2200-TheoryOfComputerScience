...
accept_file = open("unit_tests/inputs/1_9b_accepts.txt", "r")
reject_file = open("unit_tests/inputs/1_9b_rejects.txt", "r")
...
### Test Inputs ###
from assignment02_main import prob_1_9b

for w in accept_list:
    if (w not in prob_1_9b):
        print(f'FAILED ON INPUT: \'{w}\'')
    assert w in prob_1_9b

for w in reject_list:
    if (w in prob_1_9b):
        print(f'FAILED ON INPUT: \'{w}\'')
    assert w not in prob_1_9b
