...
accept_file = open("unit_tests/inputs/1_10b_accepts.txt", "r")
reject_file = open("unit_tests/inputs/1_10b_rejects.txt", "r")
...
### Test Inputs ###
from assignment02_main import prob_1_10b

for w in accept_list:
    if (w not in prob_1_10b):
        print(f'FAILED ON INPUT: \'{w}\'')
    assert w in prob_1_10b

for w in reject_list:
    if (w in prob_1_10b):
        print(f'FAILED ON INPUT: \'{w}\'')
    assert w not in prob_1_10b
